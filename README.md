...
### 高精度模式（BS-RoFormer）内存治理

高精度模式不再把整段音频交给模型：改为「规范 PCM 一次落盘 → 按 240 秒核心（含左右 5 秒上下文保护区）整数采样切窗 → 常驻 BS-RoFormer 逐片分离 → 整数采样裁剪 → FFmpeg 流式拼接」：

- Python 峰值内存与视频时长无关，只与单片长度有关；按物理内存设置 4/8/16 GiB 进程树预算（85% 警告一次、100% 终止），内存超限自动降为 120 秒核心重试一次，120 秒仍超限则明确报错。Windows 另设 Job Object 硬限（软预算 115%）作突发兜底。
- 最终人声轨的总采样数与规范 PCM 严格一致（整数采样语义，误差不超过 1 采样），120 秒识别分片的 lead-in 跨界处音频连续。
- 任务期间临时占用数 GiB 磁盘（规范 PCM + 核心片 + 最终输出，94 分钟素材约 3 GB）；成功、失败、取消与超时路径均自动清理，识别结束后人声轨临时文件随会话释放。
- **高精度分离失败会停止识别并明确报错**，不再静默回退普通音频或 demucs；请查看错误、调整设置后重试或取消勾选。
- 诊断日志：`app_data_dir/logs/subtrans.log`（JSON 行：`task_id`、规范 PCM 帧数、每片采样范围、内存峰值、降片重试、拼接校验等，5MB 轮转；路径脱敏）。
